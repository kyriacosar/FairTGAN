    ...
    def __init__(self, model, input_queue):
        ...
        with tf.name_scope('optimize'):
            ...
            with tf.control_dependencies([g_min_train_op]):
                f_min_grad = opt.compute_gradients(model.f_loss, var_list=model.f_vars)
                f_min_grad_clip = [(tf.clip_by_value(grad, -5.0, 5.0), var) for grad, var in f_min_grad]
                f_min_train_op = opt.apply_gradients(f_min_grad_clip, name='f_op')

            with tf.control_dependencies([g_min_train_op]):
                y_min_grad = opt.compute_gradients(model.y_loss, var_list=model.y_vars)
                y_min_grad_clip = [(tf.clip_by_value(grad, -5.0, 5.0), var) for grad, var in y_min_grad]
                y_min_train_op = opt.apply_gradients(y_min_grad_clip, name='y_op')
