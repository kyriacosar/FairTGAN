...
from trainer import GANTrainer, SeparateGANTrainer
from data import Preprocessor, RandomZData, TGANDataFlow
...
class GraphBuilder(ModelDescBase):
    ...
    def collect_variables(self, g_scope='gen', d_scope='discrim', f_scope='fair', y_scope='class'):
        ...
        self.f_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, f_scope)
        self.y_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, y_scope)

        if not (self.g_vars or self.d_vars or self.f_vars or self.y_vars):
            raise ValueError('There are no variables defined in some of the given scopes')

    def build_losses(self, logits_real, logits_fake, logits_s_pred, logits_s_true, logits_y_pred, logits_y_true, extra_g=0, l2_norm=0.00001):
        ...
        with tf.name_scope("GAN_loss"):
            ...
            score_y_pred = tf.sigmoid(logits_y_pred)
            tf.summary.histogram('score-y-pred', score_y_pred)
            ...
            with tf.name_scope("class"):
                y_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits( \
                    logits=logits_y_pred, \
                    labels=logits_y_true), \
                    name='loss_class'
                )

                y_pred_acc = tf.reduce_mean(tf.cast(score_y_pred < 0.5, tf.float32), name='accuracy_y')

                # y_loss = y_loss + tf.contrib.layers.apply_regularization(tf.contrib.layers.l2_regularizer(l2_norm), tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "class"))

                self.y_loss = tf.identity(y_loss, name='loss')
            ...
            add_moving_summary(g_loss, extra_g, self.g_loss, self.d_loss, d_pos_acc, d_neg_acc, - self.f_loss, s_pred_acc, self.y_loss, y_pred_acc, decay=0.)
    ...
    def inputs(self):
        ...
        for col_id, col_info in enumerate(self.metadata['details']):
            if col_info['type'] == 'value':
                ...
                inputs.append(InputDesc(tf.float32, (self.batch_size, 1), 'input%02dvalue' % col_id))
                inputs.append(InputDesc(tf.float32, (self.batch_size, gaussian_components), 'input%02dcluster' % col_id))
        ...
        with tf.variable_scope('LSTM'):
            ...
            for col_id, col_info in enumerate(self.metadata['details']):
                if col_info['type'] == 'value':
                    ...
                    raise ValueError("self.metadata['details'][{}]['type'] must be either `category` or `values`. Instead it was {}.".format(col_id, col_info['type']))
    ...
    def discriminator(self, vecs):
        ...
        for i in range(self.num_dis_layers):
            with tf.variable_scope('dis_fc{}'.format(i)):
                if i == 0:
                    logits = FullyConnected('fc', logits, self.num_dis_hidden, nl=tf.identity, kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
                else:
    ...
    @auto_reuse_variable_scope
    def classifier(self, vecs):
        r"""Build fairness metrics component.
        """
        logits = tf.concat(vecs, axis=1)
        for i in range(self.num_dis_layers):
            with tf.variable_scope('class_fc{}'.format(i)):
                if i == 0:
                    logits = FullyConnected('fc', logits, self.num_dis_hidden, nl=tf.identity, kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
                else:
                    logits = FullyConnected('fc', logits, self.num_dis_hidden, nl=tf.identity)
        ...
        return FullyConnected('class_fc_top', logits, 1, nl=tf.identity)
    ...
    def build_graph(self, *inputs):
        ...
        ptr = 0
        for col_id, col_info in enumerate(self.metadata['details']):
            if col_id == self.sensitive_column:
                ptr += 1
            elif col_info['type'] == 'category':
        ...
        if self.training:
            for col_id, col_info in enumerate(self.metadata['details']):
                if col_id == self.sensitive_column:
                    ...
                    dist = tf.reduce_sum(vecs_gen[g_ptr], axis=0)
                    dist = dist / tf.reduce_sum(dist)

                    real = tf.reduce_sum(vecs_pos[p_ptr], axis=0)
        ...
        fair_true = tf.cast(inputs[self.sensitive_column], 'float32')
        with tf.variable_scope('fair'):
        	# vecs_gen contains all features and output variable.
        	# only x should be passed
        	# s and y should be removed from vecs_gen
        	# output is s prediction      	
        	fair_pred = self.fairness(x_gen)
            
        label_true = tf.cast(inputs[-1], 'float32')
        with tf.variable_scope('class'):
        	# vecs_gen contains all features and output variable.
        	# only x should be passed
        	# s and y should be removed from vecs_gen
        	# output is s prediction
        	label_pred = self.classifier(x_gen)
            
        disc_gen = vecs_gen.copy()
        del disc_gen[12]

        with tf.variable_scope('discrim'):
            discrim_pos = self.discriminator(vecs_pos)
            discrim_neg = self.discriminator(disc_gen)
            
        self.build_losses(discrim_pos, discrim_neg, fair_pred, fair_true, label_pred, label_true, extra_g=KL, l2_norm=self.l2norm)
...
class TGANModel:
    ...
    def __init__(
        self, continuous_columns, sensitive_column, output='output', gpu=None, max_epoch=5, steps_per_epoch=10000,
        ...
        num_dis_layers=1, num_dis_hidden=100, optimizer='AdamOptimizer', trainer='GANTrainer'
    ):
        """Initialize object."""
        # Output
    ...
    def fit(self, data):
        ...
        if self.trainer == 'GANTrainer':
        	...
        	raise ValueError('Incorrect trainer name. Use GANTrainer or SeparateGANTrainer')
