...
Contains `Adult Data Prep.ipynb` Jupyter Notebook which is used to prepare and store the original `Adult` training 
and test sets in the Base and Fair TGAN model folders and Experiments folders and the original `Adult` datasets.

Preparation and saving of all necessary datasets happens just by running `Adult Data Prep.ipynb`.

## Trained Models

This directory files for the Base TGAN model and the 6 Fair TGAN models described in the thesis. These are info 
and note files, the generated dataset of each model used in the experiments and the two files containing the code
that builds the model. Those files are `model.py` and `trainer.py`. These files are necessary if the model needs
to be loaded again.

## TGAN-Master

Contains two folders for the `Base TGAN` and `Fair TGAN` models. Both folders have the same structure and the
following applies for both.

For each model two folders should be a consideration if a new model needs to be trained or an old model needs 
to be loaded. These folders are `tgan` and `examples`.

Folder `tgan` contains the base code for the models. From those files only `model.py` and `trainer.py` were
changed. All trained models referred in the thesis have these files stored in each of their folders in 
`Trained Models` directory described previously. To load an old model, its two code files, `model.py` and 
`trainer.py` should be copied from their folder in this folder.

Folder `examples` contains two Jupyter Notebooks. One is a new model trainer and the other in a model loader.
This folder also contains the `demo` folder where the model files are. These files are necessary if an older 
model needs to be loaded. All models that were trained and used in the thesis have their files in this folder
to allow them to be easily loaded again if necessary using the `Base TGAN Model Loader.ipynb` or 
`Fair TGAN Model Loader.ipynb` notebooks. Each notebook generates a dataset from the model and stores it in the 
`ML and Fairness Experiments` folder described later.

## ML and Fairness Experiments

All results and graphs presented in the thesis can be found in the notebooks in this folder. There's a separate
notebook for the original dataset, Base TGAN and Fair TGAN Models 3,4 and 6. These notebooks contain all the 
trained classifiers and their results as well as the executed fairness tests and their measures for each model.
All datasets are contained in the `Datasets` folder.


# Guide from original TGAN Repository
