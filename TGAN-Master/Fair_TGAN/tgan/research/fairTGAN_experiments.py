...
def fit_score_model(
        name, model_kwargs, train_data, test_data, continuous_columns, semsitive_column,
    ...
    for index, kwargs in enumerate(model_kwargs):
        ...
        model = TGANModel(continuous_columns, sensitive_column, output=output, **kwargs)
        ...
        score, p_rules = evaluate_classification(sampled_data, test_data, continuous_columns, sensitive_column)
...
def run_experiment(
    name, epoch, steps_per_epoch, sample_rows, train_csv, continuous_columns, sensitive_column,
    ...
    return fit_score_model(
        ...
        continuous_columns, sensitive_column, sample_rows, store_samples
    )
