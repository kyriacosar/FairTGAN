...
class GANTrainer(TowerTrainer):
    ...
    def __init__(self, model, input_queue):
        ...
        discrim_opt, fair_opt = model.get_optimizer()

        # Define the training iteration by default, run one d_min after one g_min
        with tf.name_scope('optimize'):
            g_min_grad = discrim_opt.compute_gradients(model.g_loss, var_list=model.g_vars)
            g_min_grad_clip = [(tf.clip_by_value(grad, -5.0, 5.0), var) for grad, var in g_min_grad]
            g_min_train_op = discrim_opt.apply_gradients(g_min_grad_clip, name='g_op')

            with tf.control_dependencies([g_min_train_op]):
                f_min_grad = fair_opt.compute_gradients(model.f_loss, var_list=model.f_vars)
                f_min_grad_clip = [(tf.clip_by_value(grad, -5.0, 5.0), var) for grad, var in f_min_grad]
                f_min_train_op = fair_opt.apply_gradients(f_min_grad_clip, name='f_op')

            with tf.control_dependencies([g_min_train_op]):
                d_min_grad = discrim_opt.compute_gradients(model.d_loss, var_list=model.d_vars)
                d_min_grad_clip = [(tf.clip_by_value(grad, -5.0, 5.0), var) for grad, var in d_min_grad]
                d_min_train_op = discrim_opt.apply_gradients(d_min_grad_clip, name='d_op')
         
        self.train_op = d_min_train_op
        # self.train_op = f_min_train_op
...
class SeparateGANTrainer(TowerTrainer):
    ...
    def __init__(self, model, input_queue, d_period=1, g_period=1, f_period=1):
        """Initialize object."""
        super(SeparateGANTrainer, self).__init__()
        self._d_period = int(d_period)
        self._g_period = int(g_period)
        self._f_period = int(f_period)
        if not min(d_period, g_period) == 1 and not min(f_period, g_period) == 1:
        ...
        with tf.name_scope('optimize'):
            ...
            self.f_min = opt.minimize(model.f_loss, var_list=model.f_vars, name='f_min')
